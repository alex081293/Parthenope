...
void SPIFLASH_Program(unsigned int addr, unsigned char *pBuf, unsigned int len)
{
    int index = 0;

    while (len > 0)
    {
        uint32_t writeSize = (len > PAGE_SIZE) ? PAGE_SIZE : len;
        SPIFLASH_ProgramPage(addr+index, pBuf[index], writeSize);
